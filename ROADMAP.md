...
| 单张 2048px 代理图内存 | [x] 手动通过 | 进程 RSS **50.7 MB**（增量 14.3 MB） | 单张 1080p JPEG 打开后 |
| 预加载前后各 1 张的内存增量 | [x] 手动通过 | **98–150 MB** 波动，有回落 | 49 张混合格式文件夹翻页测试 |
| 1000 张图片文件夹扫描耗时 | [x] 自测通过 | **9.6 ms** | `testFolderScanner_1000Images` |
| 1000 张缩略图生成内存峰值 | [x] 手动通过 | 峰值 **118.3 MB**，稳定 **117.7 MB** | 1000 张 JPEG/PNG 网格视图 |
| 单张 2048px 代理图内存 | [x] 自测通过 | 像素缓冲区 **7.9 MB** (1080p)；进程内 RSS 增量 ~**12.5 MB** | `PerformanceTests::testDecodeMemoryImpact_JPEG_1080p` |
| 预加载前后各 1 张的内存增量 | [ ] 待手动测 | — | 需 app 进程级 RSS 对比 |
| 1000 张图片文件夹扫描耗时 | [x] 自测通过 | **9.6 ms** | `testFolderScanner_1000Images` |
| 1000 张缩略图生成内存峰值 | [ ] 待手动测 | 100 张批量 **44 ms** / 0.44 ms 每张 | `testThumbnailBatch_100Images`；进程级 RSS 需手动
